...
static void Intro_TryShinyAnimShowHealthbox(void)
{
     ...
     && !gBattleSpritesDataPtr->healthBoxesData[BATTLE_PARTNER(gActiveBattler)].ballAnimActive)
    {
        if (!gBattleSpritesDataPtr->healthBoxesData[gActiveBattler].healthboxSlideInStarted)
        {
            ...
            {
                {
                    u16 species = GetMonData(&gPlayerParty[gBattlerPartyIndexes[2]], MON_DATA_SPECIES);
                    if (species == SPECIES_NONE)
                    {
                        FreeOamMatrix(gSprites[gBattlerSpriteIds[2]].oam.matrixNum);
                        DestroySprite(&gSprites[gBattlerSpriteIds[2]]);
                        SetHealthboxSpriteInvisible(gHealthboxSpriteIds[2]);
                        SetHealthboxSpriteInvisible(gHealthboxSpriteIds[4]);
                        SetHealthboxSpriteVisible(gHealthboxSpriteIds[4]);
                        PlayerBufferExecCompleted();
                    }
                }
            }
