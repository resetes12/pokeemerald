...
static s32 IsRequestedTypeOrEggInPlayerParty(u32 type, u32 species)
{
    ...
    else
    {
        ...
        {
            species = GetMonData(&gPlayerParty[i], MON_DATA_SPECIES2);
            if (GetTypeBySpecies(species, 1) == type || GetTypeBySpecies(species, 2)  == type) //tx_difficulty_challenges //if (gBaseStats[species].type1 == type || gBaseStats[species].type2 == type)
                return UR_TRADE_MATCH;
        }
